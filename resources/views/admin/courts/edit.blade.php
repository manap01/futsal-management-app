...
<div class="max-w-2xl mx-auto">
    <div class="mb-10">
        <h2 class="text-4xl font-headline font-black text-on-surface tracking-tight uppercase">Edit Arena</h2>
        <p class="text-on-surface-variant font-label text-sm uppercase tracking-[0.2em] mt-1">Perbarui konfigurasi {{ $court->name }}</p>
    </div>

    <div class="glass-card p-10 rounded-3xl border border-white/5 shadow-2xl">
        <form action="{{ route('admin.courts.update', $court->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="space-y-2">
            ...
            <div class="space-y-2">
                ...
                <input type="number" name="price_per_hour" value="{{ (int)$court->price_per_hour }}" 
                    class="w-full bg-surface-container border-none text-on-surface px-6 py-4 rounded-xl focus:ring-2 focus:ring-primary transition-all font-headline font-bold uppercase tracking-tight" required />
            </div>
            <div class="space-y-2">
                <label class="block font-headline font-bold text-xs uppercase tracking-widest text-primary ml-1">URL Gambar</label>
                <input type="text" name="image" value="{{ $court->image }}" 
                    class="w-full bg-surface-container border-none text-on-surface px-6 py-4 rounded-xl focus:ring-2 focus:ring-primary transition-all font-headline font-bold uppercase tracking-tight" required />
            </div>
            <div class="space-y-2">
                <label class="block font-headline font-bold text-xs uppercase tracking-widest text-primary ml-1">Deskripsi / Fitur</label>
                <textarea name="description" rows="3" 
                    class="w-full bg-surface-container border-none text-on-surface px-6 py-4 rounded-xl focus:ring-2 focus:ring-primary transition-all font-headline font-bold uppercase tracking-tight" required>{{ $court->description }}</textarea>
            </div>
            <div class="pt-4">
                <button type="submit" 
